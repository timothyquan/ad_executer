...
def compname_from_dn(computer):
    ...
    for string in computer.split(','):
        if string[:string.find('=')] in strings.keys():
            strings[string[:string.find('=')]] = strings[string[:string.find(
...
def generate_host_list(dn):
    ...
    try:
        ...
        # return empty dataframe in the instance that the dn lookup generated an exception
        return computers
    for idx, child in enumerate(ou.get_children()):
        ...
        try:
            computers.loc[idx] = {
                'fdqn': fdqn, 'ip_address': socket.gethostbyname(fdqn), 'online': False}
        except:
            computers.loc[idx] = {'fdqn': fdqn,
                                  'ip_address': 'resolution-failed', 'online': False}
        threads.append(Thread(target=is_online, args=(
            computers.loc[idx]['ip_address'], ping_results, idx)))
        threads[idx].start()
...
def psexec_runner(cmd, host, idx, results, user='', password=''):
    ...
    try:
        subproc = subprocess.run(
            command_string, capture_output=True, text=False)
...
class interface:
    def __init__(self):
        while True:
            ...
            dn = input(
                'Enter the AD dn (distinguished name) to select targets from: ')
            computer_df = pd.DataFrame(generate_host_list(dn))
            computer_df = self.__select_targets(computer_df)
            computer_df['result'] = self.__send_command(
                computer_df, command)['result']
            print(
                f'\n"{command}" ran against all target with the following results:\n')
            pprint(computer_df)
            if input('Enter "x" to exit, any other key to run again: ').lower() == 'x':
                break
    ...
    def __select_targets(self, computer_df):
        ...
        while True:
            ...
            user_input = input(
                'Enter the number index number of the machine to select, "a" for all, or "n" for none: ').lower()
            try:
        ...
        while True:
            ...
            if confirm == 'y':
                for idx, ip in enumerate(list(df['ip_address'])):
                    results.append('')
                    threads.append(Thread(target=psexec_runner,
                                          args=(cmd, ip, idx, results)))
                    threads[idx].start()
...
if __name__ == "__main__":
    logging.basicConfig(filename='ad_executer.log', 
